...
A local CLI that indexes the user's past AI conversation logs and lets you list, search, slice by Trellis task boundaries, and dump cleaned dialogue from them. OpenCode logs are not yet indexable (provider adapter pending) — when an OpenCode session is the obvious target, surface that limitation rather than guessing.

**On Oh My Pi, call `trellis-mem`, not `trellis mem`.** The upstream CLI's platform list has no `omp` (and no `kimi`), so `trellis mem` cannot see this runtime's own history — it silently returns other platforms' sessions only. The `pi` adapter cannot stand in: it reads only the first JSONL line and requires `type == "session"`, while an omp transcript opens with `title`/`pad`. `trellis-mem` is a thin local wrapper that adds the omp and kimi readers and merges them with the upstream platforms; every flag and output format is identical.

| platform | log source | read by |
|---|---|---|
| omp | `~/.omp/agent/sessions/<encoded-cwd>/*.jsonl` | local wrapper |
| kimi | `~/.kimi-code/sessions/*/session_*/agents/*/wire.jsonl` | local wrapper |
| claude | `~/.claude/projects/` | upstream CLI |
| codex | `~/.codex/sessions/` | upstream CLI |
| grok | `~/.grok/sessions/` | upstream CLI |
| pi | pi agent session root | upstream CLI |
| zcode | `~/.zcode/cli/db/db.sqlite` | upstream CLI |

Use `--platform omp` to restrict to this runtime's own history, `--platform all` (default) to search everything. If `trellis-mem` is not on PATH, run it from the customization repo:
`python3 ~/orca/projects/trellis/shared/scripts/mem/trellis_mem.py <args>`.
