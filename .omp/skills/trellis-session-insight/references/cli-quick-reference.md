...
| --------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--platform omp\|kimi\|claude\|codex\|grok\|opencode\|pi\|zcode\|all` | all               | Default `all`. `omp` and `kimi` are served by the local wrapper (`trellis-mem`); the rest come from the upstream CLI. OpenCode adapter is currently a stub — see "Caveats" below.                                                               |
